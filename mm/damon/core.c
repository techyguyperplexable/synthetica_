...
static void kdamond_reset_aggregated(struct damon_ctx *c)
{
	...
	damon_for_each_target(t, c) {
		...
		damon_for_each_region(r, t) {
			...
			/* Update age for regions tracking */
			if (r->nr_accesses == r->last_nr_accesses)
				r->age++;
			else
				r->age = 0;
...
/*
 * Check if a region meets the basic conditions for a scheme
 */
...
/* Get the watermark metric value */
...
/*
 * Returns zero if the scheme is active, non-zero time to wait otherwise
 */
...
/* Returns min wait time for watermarks across all schemes */
static unsigned long kdamond_wait_watermarks(struct damon_ctx *ctx)
{
	unsigned long min_wait_time = 0;
	struct damos *s;

	damon_for_each_scheme(s, ctx) {
		unsigned long wait_time = damos_wmark_wait_us(s);
		if (!min_wait_time || (wait_time && wait_time < min_wait_time))
...
static int kdamond_fn(void *data)
{
	...
	/* Allow freezing for Android suspend */
	set_freezable();
	...
	while (!kdamond_need_stop(ctx)) {
		...
		/* Handle freezer for Android suspend */
		try_to_freeze();

		/* Wait for watermarks if needed */
		wait_time = kdamond_wait_watermarks(ctx);
		if (wait_time) {
			kdamond_usleep(wait_time);
			continue;
		}
