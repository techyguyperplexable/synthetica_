 ...
 * checks the watermarks.  999 (99.9%) - always active.
 */
static unsigned long wmarks_high __read_mostly = 999;
 ...
 * and the reclaiming.  990 (99%) - almost always active.
 */
static unsigned long wmarks_mid __read_mostly = 990;
 ...
 * granularity reclamation logic.  1 (0.1%) - never stop.
 */
static unsigned long wmarks_low __read_mostly = 1;
...
static int damon_reclaim_turn(bool on)
{
	...
	if (err) {
		pr_err("damon_set_attrs failed: %d\n", err);
		return err;
	}

	if (monitor_region_start > monitor_region_end) {
		pr_err("invalid monitor region: start > end\n");
		return -EINVAL;
	}
	if (!monitor_region_start && !monitor_region_end &&
			!get_monitoring_region(&monitor_region_start,
				&monitor_region_end)) {
		pr_err("failed to get monitoring region\n");
		return -EINVAL;
	}
	pr_info("monitoring region: [%lx, %lx)\n",
	...
	if (!err) {
		...
		pr_info("kdamond started with pid %d\n", kdamond_pid);
		return 0;
	}
	pr_err("damon_start failed: %d\n", err);
...
static void damon_reclaim_timer_fn(struct work_struct *work)
{
	...
	if (last_enabled != now_enabled) {
		err = damon_reclaim_turn(now_enabled);
		if (!err) {
			last_enabled = now_enabled;
			pr_info("DAMON_RECLAIM turned %s\n",
				now_enabled ? "on" : "off");
		} else {
			enabled = last_enabled;
			pr_err("DAMON_RECLAIM failed to turn %s (err=%d)\n",
				now_enabled ? "on" : "off", err);
		}
	}
...
static int __init damon_reclaim_init(void)
{
	...
	pr_info("DAMON_RECLAIM initialized\n");
	return 0;
}
